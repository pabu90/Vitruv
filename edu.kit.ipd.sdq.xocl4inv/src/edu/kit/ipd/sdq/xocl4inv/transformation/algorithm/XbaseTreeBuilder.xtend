	...
	private def static dispatch Node buildExpressionTree(
		...
		val IterateNode node = switch (expression.feature.featureName) {
			...
			case "map":
				new MapNode()
			default: {
				// workaround1: handle fields (because parantheses not allowed), which MUST ONLY BE USED FOR ENUMs
				if(expression.feature instanceof JvmField 
					&& expression.memberCallTarget instanceof XMemberFeatureCall
					&& (expression.memberCallTarget as XMemberFeatureCall).feature instanceof JvmEnumerationType
				){
					val e = new FeatureNode
					e.feature = (expression.memberCallTarget as XMemberFeatureCall).feature.qualifiedName + "." + expression.feature.simpleName
					return e
				}
				
				// workaround2: handle type literals
				if(expression.feature instanceof JvmGenericType
				){
					val e = new FeatureNode
					e.feature = expression.feature.qualifiedName
					return e
				}
