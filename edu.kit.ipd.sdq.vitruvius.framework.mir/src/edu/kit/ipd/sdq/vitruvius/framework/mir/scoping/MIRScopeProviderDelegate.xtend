...
class MIRScopeProviderDelegate extends XImportSectionNamespaceScopeProvider {
	...
	override getScope(EObject context, EReference reference) {
		if (context instanceof Invariant) {
			if(reference.equals(MIRPackage.eINSTANCE.invariant_Context)){
			}
		}
		...
		return if (classifier == null)
			...
			EObjectDescription.create(
				QualifiedName.create(imp.name).append(
					qualifiedNameProvider.getFullyQualifiedName(classifier).skipFirst(1)),
				classifier
			)
	}
	...
	def createFeatureCallRootElementScope(EObject context) {
		val elementsInScope = EMFHelper.getContainerHierarchy(context, true).filter [
			it instanceof FeatureMapping || it instanceof ClassMapping
		]

		return new SimpleScope(IScope.NULLSCOPE, elementsInScope.map [
			switch it {
				FeatureMapping:
					mappedElements.filter(FeatureCall).filter[it.name != null].map [
						EObjectDescription.create(it.name, it)
					]
				ClassMapping:
					mappedElements.filter(NamedEClass).filter[it.name != null].map [
						EObjectDescription.create(it.name, it)
					]
			}
		].flatten)
	...
	def createFeatureDescriptions(EClass eClass) {
		return eClass.getEAllStructuralFeatures.map [
			val qualifiedName = QualifiedName.create(it.name)
	...
	def getContainingNamedEClassScope(FeatureCall feature) {
		...
		for (var container = feature.eContainer; container != null; container = container.eContainer) {
			if (container instanceof ClassMapping) {
				resultScope = new SimpleScope(resultScope, container.mappedElements.filter(NamedEClass).filter [
					name != null
				].map [ namedClass |
